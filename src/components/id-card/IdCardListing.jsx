...
export default function IdCardListing({
  students,
  loading,
  error,
  onRetry,
    ...
    <>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center gap-3 mb-3">
        <div className="sm:w-44 shrink-0">
          <Dropdown
            ...
            placeholder={t("idCard.listing.placeholders.campus")}
          />
        </div>
        <div className="sm:w-40 shrink-0">
          <Dropdown
            ...
            placeholder={t("idCard.listing.placeholders.class")}
          />
        </div>
        <div className="sm:w-40 shrink-0">
          <Dropdown
            ...
            placeholder={t("idCard.listing.placeholders.section")}
          />
        </div>
        <div className="flex-1 min-w-0">
          <SearchBar
            value={search}
            onChange={setSearch}
            placeholder={t("idCard.listing.searchPlaceholder")}
          />
        </div>
      </div>
      ...
      {!selectedCampus && !loading && (
        <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="font-medium text-gray-600">{t("idCard.listing.selectCampusTitle")}</p>
          <p className="text-sm text-gray-400 mt-1">{t("idCard.listing.selectCampusSubtitle")}</p>
        </div>
      )}
      ...
      {selectedCampus && !loading && filteredStudents.length > 0 && (
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          <div className="flex items-center gap-3">
            ...
            >
              {allSelected ? t("idCard.listing.deselectAll") : t("idCard.listing.selectAll")}
            </button>
            {someSelected && (
              <span className="text-xs bg-blue-50 text-blue-700 font-medium px-2.5 py-0.5 rounded-full">
                {t("idCard.listing.selected", { count: selectedStudentIds.length })}
              </span>
            )}
            {!someSelected && (
              <span className="text-xs text-gray-400">
                {t("idCard.listing.studentCount", { count: filteredStudents.length })}
              </span>
            )}
          </div>
          <Button onClick={onGenerate} disabled={!someSelected}>
            {t("idCard.listing.generate", { count: selectedStudentIds.length })}
          </Button>
        </div>
      )}
      ...
      {selectedCampus && !loading && (students ?? []).length === 0 && (
        <div className="flex flex-col items-center justify-center py-16 text-center border border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="font-medium text-gray-600">{t("idCard.listing.noStudentsTitle")}</p>
        </div>
      )}
      ...
      {selectedCampus && !loading && (students ?? []).length > 0 && filteredStudents.length === 0 && (
        <div className="flex flex-col items-center justify-center py-12 text-center border border-dashed border-gray-200 rounded-2xl">
          ...
          <p className="font-medium text-gray-600">{t("idCard.listing.noMatchesTitle")}</p>
          <p className="text-sm text-gray-400 mt-1">{t("idCard.listing.noMatchesSubtitle")}</p>
        </div>
      )}
